        ...
        <div class="bottom">
          <div class="note">
            {{ userInfo ? (ETHConvert ? formatNumber6(parseFloat(userInfo.balance_eth)) :
              formatNumber2(parseFloat(userInfo.balance_usdt))) : 0 }} {{ ETHConvert ? 'ETH' : 'USDT' }} {{
    languagePack.swap_index_title3 }}
      ...
      <div class="button-review">
        <button @click="isDisplaySwapsReview = true"
          ...
          :disabled="userInfo ? (ETHConvert && balanceConvert > 0 && (balanceConvert <= userInfo.balance_eth) ? false : (!ETHConvert && balanceConvert > 0 && (balanceConvert <= userInfo.balance_usdt) ? false : true)) : true">{{
            languagePack.swap_index_title7 }}</button>
      </div>
    </div>
    <SwapsReview v-if="isDisplaySwapsReview" :ETHConvert="ETHConvert" :balanceConvert="balanceConvert" :eth="eth"
      :address="userInfo.address" @close-popup="isDisplaySwapsReview = false" @resetSwaps="resetSwaps()" />
  </div>
</template>
